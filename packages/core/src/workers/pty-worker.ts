...
class PtyWorker {
  ...
  private handleStart(message: StartMessage) {
    ...
    try {
      ...
      const options = getPtyOptions(message.worktreePath, message.cols, message.rows, message.setLocaleVariables)
  ...
  private async getPtyDiagnostics(): Promise<{
    ...
    if (process.platform === 'darwin' || process.platform === 'linux') {
      ...
      try {
        ...
        const totalOutput = execSync(`lsof -p ${pid} 2>/dev/null | grep -E "/dev/tty|/dev/ptmx" | wc -l`, {
          encoding: 'utf-8',
        })
