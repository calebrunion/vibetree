...
export function ProjectProvider({ children }: ProjectProviderProps) {
  ...
  const removeProject = (id: string) => {
    ...
    if (project) {
      ...
      Promise.all(pathsToTerminate.map((path) => window.electronAPI.shell.terminateForWorktree(path)))
        .then((results) => {
  ...
  const setSelectedWorktree = (projectId: string, worktreePath: string | null) => {
    setProjects((prev) => prev.map((p) => (p.id === projectId ? { ...p, selectedWorktree: worktreePath } : p)))
  }
