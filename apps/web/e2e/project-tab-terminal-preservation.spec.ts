...
test.describe('Project Tab Terminal Preservation', () => {
  ...
  test('terminal should remain functional after switching project tabs multiple times', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      expect(dims1!.height, `Terminal height is 0 after switching to project 1 (iteration ${i + 1})`).toBeGreaterThan(
        50
      )
      ...
      expect(dims2!.height, `Terminal height is 0 after switching to project 2 (iteration ${i + 1})`).toBeGreaterThan(
        50
      )
    }
